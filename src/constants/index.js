...
// "All" tab is UI-only — not stored as a category
export const PLAN_CATEGORIES = CATEGORIES.filter(c => c.id !== 'all')
...
// ─── BADGES ──────────────────────────────────────────────────────────────────
export const BADGES = [
  { id: 'foodies',    emoji: '🍜', label: 'Foodies',        desc: '3+ food plans done',    threshold: (plans) => plans.filter(p => p.category_id === 'food'    && p.status === 'done').length >= 3 },
  { id: 'homebodies', emoji: '🏠', label: 'Homebodies',     desc: '3+ stay-in plans done', threshold: (plans) => plans.filter(p => p.category_id === 'stayin'  && p.status === 'done').length >= 3 },
  { id: 'adventurers',emoji: '🧗', label: 'Adventurers',    desc: '3+ outdoor plans done', threshold: (plans) => plans.filter(p => p.category_id === 'outdoor' && p.status === 'done').length >= 3 },
  { id: 'travelers',  emoji: '✈️', label: 'Globetrotters',  desc: '2+ travel plans done',  threshold: (plans) => plans.filter(p => p.category_id === 'travel'  && p.status === 'done').length >= 2 },
  { id: 'creators',   emoji: '🎨', label: 'Creative Souls', desc: '2+ creative plans done',threshold: (plans) => plans.filter(p => p.category_id === 'creative'&& p.status === 'done').length >= 2 },
  { id: 'night',      emoji: '🦉', label: 'Night Owls',     desc: '3+ nightlife plans',    threshold: (plans) => plans.filter(p => p.category_id === 'nightlife'&& p.status === 'done').length >= 3 },
  { id: 'starters',   emoji: '🌱', label: 'Just Started',   desc: 'Added first plan',      threshold: (plans) => plans.length >= 1 },
  { id: 'dreamers',   emoji: '🌟', label: 'Dreamers',       desc: '3+ bucket list items',  threshold: (plans) => plans.filter(p => p.category_id === 'bucket').length >= 3 },
]
